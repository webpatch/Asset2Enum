...
ASSET_EXTENSION = ".xcassets"
IMAGE_EXTENSION = ".imageset"
FILE_NAME = "AssetImages.swift"
LOCK_FILE = "asset.lock"


class Handler:
    def __init__(self):
        self.project_name = self.get_project_name()
        self.xcassets_name = self.get_xcassets_name()
        self.xcassets_path = os.path.join(self.project_name, self.xcassets_name)
        self.swift_path = os.path.join(self.project_name, FILE_NAME)
        self.pbxproj_path = self.project_name + PROJECT_EXTENSION + '/project.pbxproj'

    @staticmethod
    def get_project_name():
        for i in os.listdir("."):
            if i.endswith(PROJECT_EXTENSION):
                return i.replace(PROJECT_EXTENSION, "")
        raise Exception("Can't find project file")

    def get_xcassets_name(self):
        for i in os.listdir(self.project_name):
            if i.endswith(ASSET_EXTENSION):
                return i
        raise Exception("Can't find xcassets file")

    def write_enums_file(self, content):
        file_template = 'import UIKit\n' \
                        'extension UIImage {\n' \
                        '\tenum Asset : String {\n$enum\t}\n\n' \
                        '\tconvenience init!(asset: Asset) {\n\t\tself.init(named: asset.rawValue)\n\t}\n}\n'
        try:
            with open(self.swift_path, "w") as f:
                f.write(file_template.replace("$enum", content))
                print("write swift file success")
        except Exception:
            print("write swift file failed")

    def update_lock_time(self):
        try:
            with open(LOCK_FILE, "w") as f:
                json.dump({"modfiyTime": os.path.getmtime(self.xcassets_path)}, f)
                print("update lock file success")
        except Exception:
            print("update lock file failed")
    ...
    def is_need_update(self):
        ...
        try:
            with open(LOCK_FILE, 'r') as f:
                data = json.load(f)
                if data["modfiyTime"] == os.path.getmtime(self.xcassets_path):
                    is_lock_ok = True
        except Exception:
            pass
        is_swift_ok = os.path.exists(self.swift_path)

        return not (is_lock_ok and is_swift_ok)

    # get images name
    def scan_images_content(self):
        return [os.path.basename(root).replace(IMAGE_EXTENSION, "") for root, _, _ in os.walk(self.xcassets_path) if
                root.endswith(IMAGE_EXTENSION)]

    def generate_enums_case(self):
        enum_str = ""
    ...
    def add_swift_to_project(self):
        project = XcodeProject.Load(self.pbxproj_path)
        # add file
        root_group = project.get_or_create_group(self.project_name)
        infos = project.add_file_if_doesnt_exist(self.swift_path, root_group)
        if len(infos) != 0:
            print("%s added into project" % FILE_NAME)
            project.save()
        else:
            print("%s already in project" % FILE_NAME)

    def update_enum_file(self, force=False):
        if not force:
            if not self.is_need_update():
                print("no need to update")
                return
        self.write_enums_file(self.generate_enums_case())
        self.update_lock_time()
        self.add_swift_to_project()

    def add_run_script(self):
        project = XcodeProject.Load(self.pbxproj_path)
        project.add_run_script(self.project_name, "/usr/bin/python %s -u" % __file__,
                               name="Auto generate xcassets enums", insert_before_compile=True)
        project.save()
...
if __name__ == "__main__":
    ...
    handler = Handler()
    parser = argparse.ArgumentParser("Auto generator xcassets enums")
    parser.add_argument("-i", help="init", action='store_true')
    parser.add_argument("-u", help="update", action='store_true')
    parser.add_argument("-f", help="force update", action='store_true')
    args = parser.parse_args()

    if args.i:
        handler.add_run_script()
        handler.update_enum_file()

    if args.u:
        handler.update_enum_file()

    if args.f:
        handler.update_enum_file(force=True)
